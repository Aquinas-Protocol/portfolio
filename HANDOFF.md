...
- **Accessibility 92 → 100:** added the `<main>` landmark (`BaseLayout.astro`), fixed homepage heading order (`.sectHead` divs → `<h2>`, "RECENT ACTIVITY" `<h4>` → `<h3>` + its CSS selector), and lightened the `.pCard .mh` card-header labels (`--dim` → `--muted`) to clear WCAG color-contrast. **Full-site a11y sweep — DONE (2026-06-01):** all 12 routes now Lighthouse a11y **100**. Applied the homepage approach per route — promoted skipped headings (kept classes; updated matching `global.css` selectors so visuals are byte-identical), dropped the duplicate `<main>` nested in `systems/[slug].astro`, gave the placeholder `role="tablist"` tabs `role="tab"` (fixes `aria-required-children`), and lightened the two `--dim`-on-solid texts (`.sysCard .telBox .tHead`, `.commitFeed .c .t`) → `--muted`. Key axe gotcha: `--dim` text over the **body radial-gradient** (`.crumb span`, `.tbd`, footer `.sig`) is scored *incomplete*, not a failure, so it was left as-is.
